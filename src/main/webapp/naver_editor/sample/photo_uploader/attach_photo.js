    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/board/naverMultiImgUploader'; 	//upload URL
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		oAjax.header("data-Type","application/json");
    ...
    function makeArrayFromString(sResString){
    	...
    	var sResStringResult=JSON.parse(sResString);
    	var sFileInfo = sResStringResult.sFileInfo;
    	fileId.push(parseInt(sResStringResult.fileId));
    	
 		try{
 			if(!sFileInfo || sFileInfo.indexOf("sFileURL") < 0){
 	    		return ;
 	    	}
 			aTemp = sFileInfo.split("&");
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  :	'/board/naverImgUploader', 
// 				location.href.replace(/\/[^\/]*$/, '') + '/file_uploader.php',	//샘플 URL입니다.
 	        sCallback : '/naver_editor/sample/photo_uploader/callback.html',
// 	        	location.href.replace(/\/[^\/]*$/, '') + '/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			setHiddenImageToForm(opener.parent.document);
		}
	}
 	
 	
// 	//customizing function
// 	function setHiddenImageToForm(parent){
// 		var form = parent.getElementById("writeForm");
// 		for(var i=0; i<fileId.length;i++){
// 			$(form).append($(`<input type="hidden" name="flId" value="${fileId[i]}" /> `))
// 		}
// 	}
 	...
 	function setHiddenImageToForm(parent){
 		var form;
 		if(parent.getElementById("writeForm")){
 			form=parent.getElementById("writeForm");
 		}
 		if(parent.getElementById("modifyForm")){
 			form = parent.getElementById("modifyForm");
 		}
 		for(var i=0; i<fileId.length;i++){
 			$(form).append($(`<input type="hidden" name="flId" value="${fileId[i]}" /> `))
 		}
